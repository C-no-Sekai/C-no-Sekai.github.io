...
let update_table = async () => {
    ...
    results.forEach(subject => {
        ...
        if (subject[1] != '-') new_html += `<td><button class='edit_btn btn btn-danger' onclick='edit_record(this)' id=${subject[0]}>Edit</button></td>`;
        new_html += "</tr>";
    })
...
let save_record = async (btn) => {
    ...
    // Check if sum=100
    // if (final_score.isNaN() || oht_score.isNaN() || project_score.isNaN() || lab_score.isNaN() || assign_score.isNaN() || quiz_score.isNaN()) return;
    // if (sum(final_score, oht_score, project_score, lab_score, assign_score, quiz_score) != 100) return;
